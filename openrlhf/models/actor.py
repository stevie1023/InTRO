...
# 生成时检测连续的 token IDs 来定位
def find_answer_sequence(token_ids,fake_answer_ids):  # 注意空格（取决于 tokenizer）
    for i in range(len(token_ids) - len(fake_answer_ids) + 1):
        if token_ids[i:i+len(fake_answer_ids)] == fake_answer_ids:
            return i
    return -1
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(pretrain_or_model, str):
            ...
            # print("E weight",self.model.get_input_embeddings().weight.shape)  # 检查权重形状
    ...
    def forward(
        ...
        output = self.model(sequences, attention_mask=attention_mask, position_ids=position_ids,output_hidden_states=output_hidden_states)
        ...
        if not self.packing_samples:
            ...
            action_log_probs = []
            action_sequences = []  # 用于存储对应的sequence
            action_logits = []  # 用于存储对应的logits
            reward_tensor_1=[]
            offset = 0
            for num_action, seq_len in zip(num_actions, packed_seq_lens):
                ...
                ###string match buz we can not add new tokens with vllm, which will cause vocab mismatch
                if tokenizer:  ###reverse的场景有solutuion:
                    # breakpoint()
                    action_sequences.append(sequences[:, start+1:end+1])  # 提取对应的sequence
                    action_logits.append(output["logits"][:, start+1:end+1, :])  # 提取对应的logits
                ...
                #     hidden_states_action.append(hidden_states[:,start:first_end_token_pos,:])
                # if reward_tensor is not None:
                #     reward_tensor_action.append(reward_tensor[:,start+1:first_end_token_pos+1])
                offset += seq_len
            action_log_probs = torch.cat(action_log_probs, dim=1)
            if tokenizer is not None:
                action_sequences = torch.cat(action_sequences, dim=1)  # 合并所有的sequence
                action_logits = torch.cat(action_logits, dim=1)  # 合并所有的logits
            # if reward_tensor is not None:
            #     reward_tensor_action = torch.cat(reward_tensor_action, dim=1)
            # if output_hidden_states!=False:
            #     hidden_states_action = torch.cat(hidden_states_action, dim=1)
        if return_output:
            return (action_log_probs, output,action_sequences,action_logits)
        else:
