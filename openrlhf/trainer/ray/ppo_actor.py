...
from openrlhf.trainer import PPOTrainer_z
...
class ActorPPOTrainer_z(PPOTrainer_z):
    def __init__(
        self,
        *args,
        vllm_engines: List = None,
...
class ActorModelRayActor(BasePPORole):
    ...
    # ###########################
        ...
        # end_id = self.tokenizer.convert_tokens_to_ids("###Answer:")
        # self.tokenizer.end_token_id = end_id

        # # Now initialize your new token
        # target_id = self.tokenizer.convert_tokens_to_ids("<<")
        # # Get current vocab size before resizing
        # original_vocab_size = actor.model.config.vocab_size
        # new_vocab_size = len(self.tokenizer)

        # # Only proceed if we actually added new tokens
        # if new_vocab_size > original_vocab_size:
        #     target_id = self.tokenizer.convert_tokens_to_ids("<<")
            
        #     # Need to gather all relevant parameters
        #     params_to_gather = [
        #         actor.model.get_input_embeddings().weight,
        #         actor.model.lm_head.weight if hasattr(actor.model, 'lm_head') else None
        #     ]
            
        #     with deepspeed.zero.GatheredParameters(params_to_gather, modifier_rank=0):
        #         if deepspeed.comm.get_rank() == 0:
        #             # Resize model embeddings
        #             linear_method = getattr(actor.model, 'linear_method', None)
        #             actor.model.resize_token_embeddings(new_vocab_size, linear_method)
                    
        #             # Initialize new token embeddings
        #             emb_weight = actor.model.get_input_embeddings().weight
        #             if end_id < emb_weight.shape[0]:  # Safety check
        #                 emb_weight.data[end_id] = emb_weight.data[target_id].clone()
                    
        #             # Initialize lm_head if exists
        #             if hasattr(actor.model, 'lm_head'):
        #                 actor.model.lm_head.weight.data[end_id] = actor.model.lm_head.weight.data[target_id].clone()
            
        #     # Update config on all ranks
        #     actor.model.config.vocab_size = new_vocab_size
        #     actor.model.config.end_token_id = end_id
            
        #     # Synchronize changes
        #     deepspeed.comm.barrier()
            
        #     # Verify alignment
        #     assert len(self.tokenizer) == actor.model.config.vocab_size, \
        #         f"Tokenizer vocab size ({len(self.tokenizer)}) doesn't match model vocab size ({actor.model.config.vocab_size})"
        # else:
        # print(f"No new tokens added, vocab size remains {original_vocab_size}")
        # print(f"'###Answer:' token ID: {self.tokenizer.convert_tokens_to_ids('###Answer:')}")
        # actor.model.custom_end_token_id = self.tokenizer.convert_tokens_to_ids('###Answer:')
...
@ray.remote(num_gpus=1)
class ActorModelRayActor_z(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
