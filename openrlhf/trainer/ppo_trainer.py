...
class PPOTrainer(ABC):
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        p_ori = F.softmax(action_logits, dim=-1).squeeze(0)
        dist = torch.distributions.Categorical(probs=p_ori)
        entropy = dist.entropy().unsqueeze(-1)  # shape: [batch_size, 1]
        mean_entropy = entropy.mean()  # scalar entropy
        ...
        # status
        status = {"policy_loss": actor_loss.item(), "actor_lr": self.actor_scheduler.get_last_lr()[0],"entropy_loss":mean_entropy.item()}
        if self.pretrain_dataloader is not None:
