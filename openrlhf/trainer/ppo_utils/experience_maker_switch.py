...
class Experience:
    ...
    action_log_probs: Optional[torch.Tensor]  # 允许为None
...
class NaiveExperienceMaker(ABC):
    ...
    def __init__(
        ...
        self.calc_action_log_probs = calc_action_log_probs
    ...
    def make_experience_list(
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            elif self.advantage_estimator in ["reinforce", "rloo", "reinforce_baseline", "group_norm","intro"]:
...
class RemoteExperienceMaker(NaiveExperienceMaker):
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        if self.calc_action_log_probs and action_log_probs is not None and base_action_log_probs is not None:
            ...
            kl = torch.zeros_like(base_action_log_probs, dtype=torch.float, device=base_action_log_probs.device)
...
class NaiveExperienceMakerBOX(ABC):
    ...
    def make_experience(self, samples: Samples, **generate_kwargs) -> Experience:
        ...
        # ray.logger.info("finish cal kl")
            ...
            kl = torch.zeros_like(base_action_log_probs, dtype=torch.float, device=action_mask.device)
...
class RemoteExperienceMakerBOX(NaiveExperienceMakerBOX):
    ...
    def make_experience(self, samples: Samples, **generate_kwargs) -> Experience:
        ...
        # ray.logger.info("finish cal kl")
            ...
            # 兜底，保证 kl 是合适形状的零张量
            if action_mask is not None:
                kl = torch.zeros_like(action_mask, dtype=torch.float, device=action_mask.device)
            else:
                kl = None
