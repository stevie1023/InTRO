...
def reward_func(queries, prompts, labels):
    ...
    for query, label in zip(queries, labels):
        response = query[len(prompts):]  # Extract the response part from the query
        base=extract_answer_math(label)
        test=extract_answer_math(response)
        if grade_answer(test, base) == True:
